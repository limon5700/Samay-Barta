    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="container mx-auto px-4 flex-grow">
        <div className="flex flex-col md:flex-row gap-8">
          <aside className="w-full md:w-1/8 lg:w-1/12 order-2 md:order-1 space-y-4"> {/* Adjusted skeleton width */}
            <Skeleton className="h-48 w-full rounded-md" /> <Skeleton className="h-32 w-full rounded-md" />
          </aside>
          <div className="w-full md:w-6/8 lg:w-10/12 order-1 md:order-2"> {/* Adjusted skeleton width */}
            <div className="mb-8 flex flex-wrap gap-2 justify-center">
              {[...Array(5)].map((_, i) => <Skeleton key={`cat-skel-${i}`} className="h-10 w-24 rounded-md" />)}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6"> {/* NewsList Skeleton, ensured 3 columns */}
              {[...Array(6)].map((_, i) => (  // Reduced to 6 for a 3-col layout example
          ...
          <aside className="w-full md:w-1/8 lg:w-1/12 order-3 md:order-3 space-y-4"> {/* Adjusted skeleton width */}
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        {isPageLoading ? (<PageSkeleton />) : (
          <>
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <aside className="w-full md:w-1/8 lg:w-1/12 order-2 md:order-1 space-y-6"> {/* Adjusted Left Sidebar Width */}
              ...
              <div className="w-full md:w-6/8 lg:w-10/12 order-1 md:order-2"> {/* Adjusted Main Content Width */}
              ...
              <aside className="w-full md:w-1/8 lg:w-1/12 order-3 md:order-3 space-y-6"> {/* Adjusted Right Sidebar Width */}
