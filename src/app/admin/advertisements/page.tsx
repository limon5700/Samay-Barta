...
import { PlusCircle, Edit, Trash2, Loader2, ExternalLink, Eye, EyeOff } from "lucide-react";
...
export default function AdvertisementsPage() {
  ...
  const handleFormSubmit = async (data: AdvertisementFormData) => {
    ...
    try {
      if (editingAd) {
        const result = await updateAdvertisement(editingAd.id, data as Partial<Omit<Advertisement, 'id' | 'createdAt'>>);
        if (result) {
      ...
      toast({ title: "Error", description: "An error occurred while saving the advertisement.", variant: "destructive" });
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      <Card className="shadow-lg rounded-xl">
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Add, edit, or delete advertisements for the site.</CardDescription>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {ads.map((ad) => (
                  <TableRow key={ad.id}>
                    <TableCell>
                      <div className="relative h-16 w-28 rounded overflow-hidden border">
                        <Image src={ad.imageUrl} alt={ad.altText || 'Ad Image'} fill style={{objectFit: 'contain'}} data-ai-hint="advertisement graphic"/>
                      </div>
                    </TableCell>
                    <TableCell>
                      <a href={ad.linkUrl} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline flex items-center gap-1">
                        {ad.linkUrl.length > 40 ? `${ad.linkUrl.substring(0,37)}...` : ad.linkUrl }
                        <ExternalLink className="h-3 w-3" />
                      </a>
                    </TableCell>
                    <TableCell>{ad.altText || "-"}</TableCell>
                    <TableCell>
