...
const checkRequiredEnvVars = (): boolean => {
  ...
  const GEMINI_API_KEY_SET = !!process.env.GEMINI_API_KEY; // Or GOOGLE_API_KEY
  const ADMIN_USERNAME_SET = !!process.env.ADMIN_USERNAME;
  const ADMIN_PASSWORD_SET = !!process.env.ADMIN_PASSWORD;

  if (!MONGODB_URI_SET) console.error("CRITICAL: MONGODB_URI is NOT SET in process.env at runtime.");
  if (!GEMINI_API_KEY_SET) console.warn("WARNING: GEMINI_API_KEY (or GOOGLE_API_KEY) is NOT SET in process.env at runtime. AI features may fail.");
  if (!ADMIN_USERNAME_SET) console.error("CRITICAL: ADMIN_USERNAME is NOT SET in process.env at runtime. .env admin login will fail.");
  if (!ADMIN_PASSWORD_SET) console.error("CRITICAL: ADMIN_PASSWORD is NOT SET in process.env at runtime. .env admin login will fail.");
  
  return MONGODB_URI_SET && ADMIN_USERNAME_SET && ADMIN_PASSWORD_SET; // Gemini key is not strictly for login
};
...
export async function loginAction(formData: FormData): Promise<{ success: boolean; error?: string; redirectPath?: string }> {
  ...
  if (!checkRequiredEnvVars()) {
     console.error("loginAction: Essential environment variables (MONGODB_URI, ADMIN_USERNAME, ADMIN_PASSWORD) are missing. Login cannot proceed securely.");
     return { success: false, error: "Server configuration error. Please contact administrator. (Error Code: ENV_MISSING_CRITICAL)" };
  }
    ...
    if (currentRuntimeAdminUsername && currentRuntimeAdminPassword) {
      ...
      if (username === currentRuntimeAdminUsername) {
        ...
        if (password === currentRuntimeAdminPassword) {
         ...
         console.log(`loginAction: Submitted username '${username}' does not match runtime .env admin username '${currentRuntimeAdminUsername}'.`);
      }
    } else {
      ...
      // Specific check if the user *tried* to log in as the INITIAL_ENV_ADMIN_USERNAME but server-side env vars are missing
...
export async function getSession(): Promise<UserSession | null> {
  ...
  if (!checkRequiredEnvVars()) {
     console.error("getSession: Essential environment variables (MONGODB_URI, ADMIN_USERNAME, ADMIN_PASSWORD) are missing. Session validation may be unreliable.");
     // Allow to proceed but with caution; some session types might still be resolvable if DB is up.
  }
